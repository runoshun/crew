...
// CheckReviewSessionRunning checks if a review session is running for the given task ID.
func CheckReviewSessionRunning(sessions domain.SessionManager, taskID int) (bool, error) {
...
// StopReviewSession stops a review session for the given task ID if it is running.
// Returns the stopped session name (empty if not running).
func StopReviewSession(sessions domain.SessionManager, taskID int) (string, error) {
	sessionName := domain.ReviewSessionName(taskID)
	running, err := sessions.IsRunning(sessionName)
	if err != nil {
		return "", fmt.Errorf("check session running: %w", err)
...
// RequireRunningReviewSession checks if a review session is running and returns ErrNoSession if not.
func RequireRunningReviewSession(sessions domain.SessionManager, taskID int) (string, error) {
	sessionName := domain.ReviewSessionName(taskID)
	running, err := sessions.IsRunning(sessionName)
	if err != nil {
		return "", fmt.Errorf("check session: %w", err)
...
// EnsureNoRunningReviewSession checks that no review session is running for the given task.
// Returns ErrSessionRunning if review session is already running.
func EnsureNoRunningReviewSession(sessions domain.SessionManager, taskID int) error {
	sessionName := domain.ReviewSessionName(taskID)
	running, err := sessions.IsRunning(sessionName)
	if err != nil {
		return fmt.Errorf("check session: %w", err)
	...
	if running {
		return fmt.Errorf("review session is already running: %w", domain.ErrSessionRunning)
	}
	return nil
}
